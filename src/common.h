...
#define FILE_CUT_SILENCE_AFTER_ACTIVE_MS 100

// 静音的音频段，切分xxx毫秒给之后有声音的音频段
#define FILE_CUT_SILENCE_BEFORE_ACTIVE_MS 50

// 最小的音频切分长度，即每个切分后音频文件不少于这个时长，最后一个除外
#define FILE_CUT_MIN_MS  (2 * 100)

// 最大的音频切分长度，即每个切分后音频文件不多于这个时长
#define FILE_CUT_MAX_MS  (5 * 1000)
...
// FRAME_SIZE = 80 * MULTI = SAMPLE_RATE  /1000 * MULTI ,

#define FRAME_SIZE (80 * MULTI)
